...
def _build_prompt(ticker: str, display_name: str | None = None) -> str:
    """为指定股票构造分析 prompt。"""
...
async def analyze_stock(
    ...
    prompt = _build_prompt(ticker, display_name=display_name)
    name = display_name or ticker
...
async def analyze_and_save(
    ...
    path = _save_to_docx(ticker, text, output_dir, display_name=display_name)
...
async def analyze_screener_results(
    ...
    Args:
        hits: 筛选结果，每个元素至少包含 "ticker"，可选 "company_name"
        output_dir: 输出目录
        model: Gemini 模型名
        delay: 调用间延迟
    ...
    for h in hits:
        ...
        if ticker in seen:
            continue
        seen.add(ticker)
        pairs.append((ticker, h.get("company_name")))
    ...
    client = await _init_client(model)
    results: list[Path] = []
    logger.info(f"开始批量分析 {len(pairs)} 只股票（模型：{model}）...")

    try:
        for i, (ticker, company_name) in enumerate(pairs, 1):
            try:
                logger.info(f"[{i}/{len(pairs)}] 分析 {ticker}...")
                path = await analyze_and_save(
                    ticker,
                    client=client,
                    model=model,
    ...
    logger.info(f"批量分析完成：{len(results)}/{len(pairs)} 只成功")
    return results
