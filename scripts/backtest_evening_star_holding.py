#!/usr/bin/env python3
"""
backtest_evening_star_holding.py
对 holding.md 所有标的进行「高位跳空十字星」策略的完整历史回测。

用法：
    python scripts/backtest_evening_star_holding.py
    python scripts/backtest_evening_star_holding.py --min-score 3
    python scripts/backtest_evening_star_holding.py --no-chart
"""
...
from stock_ana.strategies.impl.evening_star_gap import scan_history
...
OUT_DIR = OUTPUT_DIR / "evening_star" / "backtest"
...
def _render_backtest_chart(
    ...
    for _, row in hits.iterrows():
        ...
        try:
            ...
            doji_mark = "☆" if row.get("is_strict_doji") else "◇"
            date_str = str(row["signal_date"])[5:]  # MM-DD
            vol_str = f"V{row.get('d1_vol_ratio', 0):.1f}x"
            body_str = f"b{row.get('d1_body_ratio', 0)*100:.0f}%"

            label = (
                f"{date_str}\n"
                f"{doji_mark} s{row['score']} {vol_str}\n"
    ...
    add_stat_panel(ax, panel, bg_color="#E3F2FD", edge_color="#90CAF9")
    set_chart_title(fig,
        f"{market}:{sym}  {name}  —  Evening Star 回测（共{total}次，score≥{min_score}）")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evening Star 策略历史回测（holding.md）")
    ...
    logger.info(f"holding.md 共 {len(items)} 只标的，开始 Evening Star 回测 ...")
    ...
    if not all_hits:
        print("没有找到任何 Evening Star 历史信号。")
        return
    ...
    # ── 汇总统计 ──
    print("\n" + "═" * 70)
    print(f"  Evening Star 策略 历史回测汇总  (score ≥ {args.min_score}，共 {len(filt)} 次信号)")
    print("═" * 70)
