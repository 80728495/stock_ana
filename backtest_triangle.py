...
用法：
    python backtest_triangle.py
    python backtest_triangle.py --step 3
    python backtest_triangle.py --step 10
"""
...
from stock_ana.screener import screen_ascending_triangle
...
def scan_historical_triangles(
    ...
    for cutoff in range(min_data, total - 1, step):
        ...
        try:
            result = screen_ascending_triangle(df_slice)
        except Exception:
            continue
...
def plot_signals(
    ...
    for s in to_plot:
        ...
        add_plots.append(
            mpf.make_addplot(res_line, color="red", width=2.0, linestyle="dashed")
        )
        add_plots.append(
            mpf.make_addplot(sup_line, color="lime", width=2.0, linestyle="dashed")
        )
...
def run_backtest(step: int = 5, min_gap_days: int = 20):
    ...
    # ── 绘图 ──
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    plot_signals(all_signals, stock_data, OUTPUT_DIR, max_charts=30)
