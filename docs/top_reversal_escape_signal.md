...
```
周期性(build + 训练)                   每日(scan)
────────────────────                  ──────────────
build --stage all                     daily_escape_scan
  → labeled 训练集                        │ 段A: load_discovery_models() 加载已训模型
        │                                 │      predict_discovery(候选)  ← 不重训
discovery_model --train                   │ 用最新价召回持仓候选(scan_candidates)
  → data/models/top_reversal/*.json ──▶   ▼
    (入 git，其他机器直接加载)          段B: structural_status(SMC 结构)
                                           ▼
                                     状态机 advance_state → 事件 + 状态
```

- **模型持久化**：`discovery_model.py` 训练一次（每市场 lgb 5-seed + logistic，
  DISCOVERY_FEATURE_COLS）落盘为**可移植 JSON**（`data/models/top_reversal/discovery_{US,HK,CN}.json`，
  **入 git**）。每日扫描 `load_discovery_models()` + `predict_discovery()` 直接加载预测，
  **不再运行时重训、也不依赖训练集 CSV**；其他机器 clone 仓库即得已训模型。训练口径与
  `eval_watchlist_oos.fit_logistic/fit_lightgbm` 同源（零漂移）。数据/模型更新后重训：
...
- 训练集 `watchlist_unified_recall_candidates_labeled.csv` 由 **build 周期性**产出，
  只在**重训模型**时用；日常扫描不读它。价格数据由你的日常同步流程更新到最新交易日。
...
- 模型持久化：`discovery_model.py`（`--train` 训练落盘；`load_discovery_models`/`predict_discovery`）
...
- 训练口径（一份真源）：`eval_watchlist_oos.fit_logistic/fit_lightgbm` + `predict_*`
- 训练集/评估：`build_top_candidate_research.py`（`--stage`）、`eval_watchlist_oos.py`
